...
from gap_aware_alignments.petri_net import ARTIFICIAL_END_TRANSITION_LABEL

if TYPE_CHECKING:
    from gap_aware_alignments.reachability_graph import ReachabilityGraph

# A trace is a plain sequence of activity names.
Trace = tuple[str, ...]
# An alignment is a list of moves (log_label, model_label) with ">>" for skips.
AlignmentResult = dict
...
class AlignmentStateGraph(nx.MultiDiGraph):
                ...
                if edge[2]["label"] == trace[idx]:
                    ...
                    for gap_start in gap_starts:
                        if shortest_path := reachability_graph.all_pairs_shortest_path[gap_start[1]].get(edge[0]):
                            ...
                            log_labels = [trace[i] for i in range(gap_start[0], idx)]
                            model_cost = len(shortest_path) - 1
                            model_sequences = [
                                reachability_graph.edges[u, v, 0]["firing_sequence"]
            ...
            if edge[2]["label"] == ARTIFICIAL_END_TRANSITION_LABEL:
                ...
                for gap_start in gap_starts:
                    if shortest_path := reachability_graph.all_pairs_shortest_path[gap_start[1]].get(edge[0]):
                        ...
                        log_labels = [trace[i] for i in range(gap_start[0], len(trace))]
                        model_cost = len(shortest_path) - 1
                        model_sequences = [
                            reachability_graph.edges[u, v, 0]["firing_sequence"]
                            for u, v in nx.utils.pairwise(shortest_path)
                ...
                # Add arcs corresponding to synchronous moves (artificial end move, cost 0)
                self.add_edge(
                    (len(trace), edge[0]),
                    (len(trace) + 1, edge[1]),
                    firing_sequence=edge[2]["firing_sequence"],
    ...
    def get_optimal_alignment_costs(self) -> tuple[float, float, float, float]:
        return (
            nx.dijkstra_path_length(self, self.initial_state, self.final_state, weight="classic_cost"),
